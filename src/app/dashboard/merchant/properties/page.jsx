...
export default function AgentProperties() {
  const { agentToken, agent, merchantToken, user, userToken, BASE_URL } =
    useApp();
  ...
  // Verify property
  const verifyProperty = async (propertyId, is_verified) => {
    try {
      const res = await fetch(
        `${BASE_URL}/properties/${propertyId}/set-verified`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        toast.success(
          `Property ${is_verified ? "verified" : "unverified"} successfully`
        );
        fetchAllProperties();
      } else {
    ...
    <div className="p-6">
      ...
      <div className="flex flex-col md:flex-row md:justify-end md:items-center mb-6 gap-4">
        <Link
          ...
          className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition-all duration-300"
        >
          <FaPlus /> Create
        </Link>
      </div>
